...
def get_script_icon(item_id):
    """item_icons.json: item id -> script 'Icon' value (B42 scripts).
    Falls back to Base.<rawname> for evolving/mod items redefined under Base."""
    global _ICON_INDEX_CACHE
    if _ICON_INDEX_CACHE is None:
        try:
            with open("/var/lib/zomboclat/item_icons.json", "r", encoding="utf-8") as f:
                _ICON_INDEX_CACHE = json.load(f)
        except Exception:
            _ICON_INDEX_CACHE = {}
    hit = _ICON_INDEX_CACHE.get(item_id.lower())
    if hit:
        return hit
    raw = item_id.split(".")[-1] if "." in item_id else item_id
    return _ICON_INDEX_CACHE.get("base." + raw.lower())
...
def extract_inventory_from_blob(data):
    ...
    def make_entry(item_id, count=1, display=None):
        ...
        if not icon_file:
            sicon = get_script_icon(item_id)
            if sicon:
                icon_file = sicon if sicon.endswith('.png') else None
...
def _make_child_entry(display, catalog_map):
    ...
    return {
        ...
        'mod_name': cat_info.get('mod_name'),
        'icon_file': cat_info.get('icon_file'),
        'icon': get_script_icon(item_id) if item_id else None,
        'is_container': False,
